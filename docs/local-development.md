...
说明：
- `NUXT_PUBLIC_API_BASE_URL` 是浏览器和 Nuxt SSR 进程共用的 public base URL。需要完整测试 SSR 登录态页面时，优先使用 `make backend-dev` + `make frontend-dev` 直连启动；Compose 更适合作为容器镜像 smoke test。
