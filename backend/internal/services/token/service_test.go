...
func (f fakeTokenAppStore) GetByIDForUser(ctx context.Context, id, userID uuid.UUID) (*model.Application, error) {
	if f.app == nil {
		return nil, model.ErrNotFound
	}
	return f.app, nil
...
func TestTokenOnlyApprovedApplicationCanCreate(t *testing.T) {
	...
	_, err := svc.Create(context.Background(), pending.UserID, pending.ID, "prod")
	...
	res, err := svc.Create(context.Background(), approved.UserID, approved.ID, "prod")
	if err != nil {
