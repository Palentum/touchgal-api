...
func (r *TokenRepo) Create(ctx context.Context, token model.APIToken) (*model.APIToken, error) {
	err := r.db.QueryRow(ctx, `
		...
		RETURNING id, user_id, application_id, name, token_prefix, token_hash, status, minute_limit, daily_limit, last_used_at, expires_at, revoked_at, created_at, updated_at`,
	...
	).Scan(&token.ID, &token.UserID, &token.ApplicationID, &token.Name, &token.TokenPrefix, &token.TokenHash, &token.Status, &token.MinuteLimit, &token.DailyLimit, &token.LastUsedAt, &token.ExpiresAt, &token.RevokedAt, &token.CreatedAt, &token.UpdatedAt)
	return &token, err
}

func (r *TokenRepo) ListByUser(ctx context.Context, userID uuid.UUID) ([]model.APIToken, error) {
	rows, err := r.db.Query(ctx, `
		SELECT id, user_id, application_id, name, token_prefix, token_hash, status, minute_limit, daily_limit, last_used_at, expires_at, revoked_at, created_at, updated_at
...
func (r *TokenRepo) GetByHashWithApplication(ctx context.Context, tokenHash string) (*model.TokenAuthInfo, error) {
	...
	err := r.db.QueryRow(ctx, `
		SELECT t.id, t.user_id, t.application_id, t.name, t.token_prefix, t.token_hash, t.status, t.minute_limit, t.daily_limit, t.last_used_at, t.expires_at, t.revoked_at, t.created_at, t.updated_at,
		       a.status, a.id, a.user_id
		FROM api_tokens t
		JOIN api_applications a ON a.id = t.application_id
		WHERE t.token_hash = $1`, tokenHash,
	).Scan(&info.Token.ID, &info.Token.UserID, &info.Token.ApplicationID, &info.Token.Name, &info.Token.TokenPrefix, &info.Token.TokenHash, &info.Token.Status, &info.Token.MinuteLimit, &info.Token.DailyLimit, &info.Token.LastUsedAt, &info.Token.ExpiresAt, &info.Token.RevokedAt, &info.Token.CreatedAt, &info.Token.UpdatedAt, &info.ApplicationStatus, &info.ApplicationID, &info.UserID)
...
func (r *TokenRepo) RevokeForUser(ctx context.Context, id, userID uuid.UUID) error {
	tag, err := r.db.Exec(ctx, `UPDATE api_tokens SET status = 'revoked', revoked_at = now() WHERE id = $1 AND user_id = $2 AND status <> 'revoked'`, id, userID)
	if err != nil {
		return err
	}
...
func (r *TokenRepo) RevokeByAdmin(ctx context.Context, id uuid.UUID) error {
	tag, err := r.db.Exec(ctx, `UPDATE api_tokens SET status = 'revoked', revoked_at = now() WHERE id = $1 AND status <> 'revoked'`, id)
	if err != nil {
		return err
	}
	if tag.RowsAffected() == 0 {
...
func (r *TokenRepo) get(ctx context.Context, where string, args ...any) (*model.APIToken, error) {
	...
	err := r.db.QueryRow(ctx, `
		SELECT id, user_id, application_id, name, token_prefix, token_hash, status, minute_limit, daily_limit, last_used_at, expires_at, revoked_at, created_at, updated_at
		FROM api_tokens `+where, args...).Scan(&token.ID, &token.UserID, &token.ApplicationID, &token.Name, &token.TokenPrefix, &token.TokenHash, &token.Status, &token.MinuteLimit, &token.DailyLimit, &token.LastUsedAt, &token.ExpiresAt, &token.RevokedAt, &token.CreatedAt, &token.UpdatedAt)
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, model.ErrNotFound
	}
	return token, err
...
func scanTokens(rows pgx.Rows) ([]model.APIToken, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&token.ID, &token.UserID, &token.ApplicationID, &token.Name, &token.TokenPrefix, &token.TokenHash, &token.Status, &token.MinuteLimit, &token.DailyLimit, &token.LastUsedAt, &token.ExpiresAt, &token.RevokedAt, &token.CreatedAt, &token.UpdatedAt); err != nil {
			return nil, err
		}
