<template>
  <article class="tg-doc-article tg-api-doc">
    ...
    <section class="tg-card-dark">
      ...
      <p class="tg-muted" style="margin-top: 12px;">业务接口不要在不可信浏览器环境暴露 token。示例 base 取自 <code>NUXT_PUBLIC_API_BASE_URL</code>，同源相对路径会自动拼接当前域名。</p>
...
// 文档示例需要绝对 origin 才能复制为可执行 curl。绝对 base 直接使用；同源相对 base（如 /api 反代）
// 在 SSR 取当前请求 origin 拼成绝对 URL，与 useApi() 的同源请求语义一致。
const exampleBaseURL = computed(() => {
  const base = rawBaseURL.value
  try {
  ...
  if (base.startsWith('/')) {
    ...
    return origin ? `${origin.replace(/\/$/, '')}${base}` : '<你的部署地址>'
  }
  return '<你的部署地址>'
})
const requestExample = computed(() => props.doc.requestExample(exampleBaseURL.value))
